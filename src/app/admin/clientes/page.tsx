...
export default function Clientes() {
  ...
  const [form, setForm] = useState<Cliente>({
    ...
    enderecoEntrega: { cep:'', rua:'', bairro:'', cidade:'', estado:'', numero:'', complemento:'' },
    telefoneFixo: '',
    telefoneCelular: '',
    cnpj: '',
    inscricaoEstadual: '',
  ...
  const auth = getAuth(); // pega o usuário logado
    ...
    const loadClientes = async () => {
        ...
        const data = snap.docs.map(doc => {
            ...
            enderecoEntrega: d.enderecoEntrega || { cep:'', rua:'', bairro:'', cidade:'', estado:'', numero:'', complemento:'' },
            ...
            nomeContatoCompra: d.nomeContatoCompra || '',
            diasRecebimento: d.diasRecebimento || [],
            horarioRecebimento: d.horarioRecebimento || ''
            };
        });
  ...
  // 🔹 Load clientes do Firestore
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        const clienteToSave = {
            ...
            enderecoEntrega: usarMesmoEndereco ? form.endereco : form.enderecoEntrega
        };
        ...
        try {
            ...
            if (!cliente.id) throw new Error("Cliente sem ID!");
            await setDoc(doc(db, 'clientes', cliente.id), clienteToSave);
            const updatedClientes = [...clientes];
            updatedClientes[editingIndex] = { ...clienteToSave, id: cliente.id };
            setClientes(updatedClientes);
            setEditingIndex(null);
            } else {
            // Cria novo cliente
            const newDocRef = doc(collection(db, 'clientes'));
            await setDoc(newDocRef, clienteToSave);
            setClientes([...clientes, { ...clienteToSave, id: newDocRef.id }]);
            }
            resetForm();
        } catch (error) {
  ...
  const handleEdit = (index: number) => {
    ...
    setForm(cliente);
    setUsarMesmoEndereco(JSON.stringify(cliente.endereco) === JSON.stringify(cliente.enderecoEntrega));
  ...
  // Formata CEP: 12345678 → 12345-678
    ...
    const numeros = cep.replace(/\D/g, "");
        if (numeros.length === 8) {
            ...
            if (!data.erro) {
                const enderecoKey = isEntrega ? "enderecoEntrega" : "endereco";
            ...
            {showForm && (
                <form onSubmit={handleSubmit} className="mb-8 p-4 border border-gray-300 rounded">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        <div className="md:col-span-2">
                            <label className="block text-sm font-medium text-gray-700">
                                <input
                                    ...
                                    onChange={(e) => setUsarMesmoEndereco(e.target.checked)}
                                    className="mr-2"
                                />
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                                    <div>
                                        ...
                                        <input
                                            ...
                                            onChange={(e) => {
                                                const formattedCep = formatarCep(e.target.value);
                                                setForm({ ...form, enderecoEntrega: { ...form.enderecoEntrega, cep: formattedCep } });
                                                buscarCep(e.target.value.replace(/\D/g, ''), true, setForm); // ⚠️ aqui também
                                            }}
                                    ...
                                    <div className="md:col-span-2">
                                        ...
                                        <input
                                            ...
                                            onChange={(e) => setForm({ ...form, enderecoEntrega: { ...form.enderecoEntrega, rua: e.target.value } })}
                                            className="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md"
                                            disabled={cepValido.endereco}
                                        />
                                    </div>
                                    <div>
                                        ...
                                        <input
                                            ...
                                            onChange={(e) => setForm({ ...form, enderecoEntrega: { ...form.enderecoEntrega, numero: e.target.value } })}
                                            className="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md"
                                        />
                                    </div>
                                    <div>
                                        ...
                                        <input
                                            ...
                                            onChange={(e) => setForm({ ...form, enderecoEntrega: { ...form.enderecoEntrega, complemento: e.target.value } })}
                                            className="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md"
                                        />
                                    </div>
                                    <div>
                                        ...
                                        <input
                                            ...
                                            onChange={(e) => setForm({ ...form, enderecoEntrega: { ...form.enderecoEntrega, bairro: e.target.value } })}
                                            className="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md"
                                            disabled={cepValido.endereco}
                                        />
                                    </div>
                                    <div>
                                        ...
                                        <input
                                            ...
                                            onChange={(e) => setForm({ ...form, enderecoEntrega: { ...form.enderecoEntrega, cidade: e.target.value } })}
                                            className="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md"
                                            disabled={cepValido.endereco}
                                        />
                                    </div>
                                    <div>
                                        ...
                                        <input
                                            ...
                                            onChange={(e) => setForm({ ...form, enderecoEntrega: { ...form.enderecoEntrega, estado: e.target.value } })}
                                            className="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md"
                                            maxLength={2}
                                            disabled={cepValido.endereco}
                                        />
                        ...
                        <div>
                            <label className="block text-sm font-medium text-gray-700">Nome de Contato de Compra</label>
                            <input
                                type="text"
                        ...
                        <div>
                            <label className="block text-sm font-medium text-gray-700">Telefone Contato de Compra</label>
                            <input
                                type="text"
                                value={form.nomeContatoCompra}
                                onChange={(e) => setForm({ ...form, nomeContatoCompra: formatarTelefone(e.target.value) })}
                                className="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md"
                                placeholder="(XX) XXXXX-XXXX"
                            />
                        </div>
                        <div className="md:col-span-2">
                            <label className="block text-sm font-medium text-gray-700 mb-2">Dias de Recebimento</label>
                            <div className="grid grid-cols-7 gap-2">
                                {['Seg', 'Ter', 'Qua', 'Qui', 'Sex', 'Sáb', 'Dom'].map(dia => (
                                    <label key={dia} className="flex items-center">
                                        <input
                                            type="checkbox"
                                            checked={form.diasRecebimento.includes(dia)}
                                            onChange={(e) => {
                                                if (e.target.checked) {
                                                    setForm({ ...form, diasRecebimento: [...form.diasRecebimento, dia] });
                                                } else {
                                                    setForm({ ...form, diasRecebimento: form.diasRecebimento.filter(d => d !== dia) });
                                                }
                                            }}
                                            className="mr-1"
                                        />
                        ...
                        <div className="md:col-span-2">
                            <label className="block text-sm font-medium text-gray-700">Horário de Recebimento</label>
                            <input
                                type="text"
                                value={form.horarioRecebimento}
                                onChange={(e) => setForm({ ...form, horarioRecebimento: e.target.value })}
                                className="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md"
                        ...
                        {aberto && (
                            <>
                                <div className='border-b border-gray-300 p-4'>                            
                                    ...
                                    <div className='flex flex-col border border-gray-300 p-2 rounded-md mb-2'>
                                    ...
                                    <div className='flex flex-col border border-gray-300 p-2 rounded-md mb-2'>
                                        <p className='text-sm text-left'><strong>Endereço Entrega</strong> </p>
                                        <p>{cliente.enderecoEntrega.rua}, {cliente.enderecoEntrega.numero} - {cliente.enderecoEntrega.bairro}, {cliente.enderecoEntrega.cidade} - {cliente.enderecoEntrega.estado}, CEP: {cliente.enderecoEntrega.cep}</p>
                                    </div> 

                                    <div className='grid grid-cols-2 mb-2 gap-2'>
                                    <div className='border border-gray-300 p-2 rounded-md '>
                                            <p><strong>Contato:</strong> {cliente.nomeContato}</p>
                                            <p><strong>Contato Compra:</strong> {cliente.nomeContatoCompra}</p>
                                    </div>
                                    <div className='border border-gray-300 p-2 rounded-md '>
                                            <p><strong>Dias Recebimento:</strong> {cliente.diasRecebimento.join(', ')}</p>
                                            <p><strong>Horário:</strong> {cliente.horarioRecebimento}</p>
                                    </div>
